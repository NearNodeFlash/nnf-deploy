...
func readConfigFile(path string) (*RepositoryConfigFile, error) {
	configFile, err := os.ReadFile(path)
	if err != nil {
		configFile, err = os.ReadFile(filepath.Join("..", path))
		if err != nil {
			return nil, err
		}
	}
...
func EnumerateDaemons(path string, handleFn func(Daemon) error) error {
	configFile, err := os.ReadFile(path)
	if err != nil {
		return err
	}
