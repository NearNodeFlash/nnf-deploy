...
parser.add_argument("--prev-ver", type=str, required=True,
...
parser.add_argument("--no-commit", "-C", action='store_true', dest='nocommit',
...
def create_apis(git, stage, project, args):
  ...
  createapis = CreateApis(args.dryrun, project, args.prev_ver, args.new_ver, CGEN.preferred_api_alias(), CGEN.module())
...
def mv_webhooks(git, stage, project, args):
  ...
  webhooks = MvWebhooks(args.dryrun, project, args.prev_ver, args.new_ver, CGEN.preferred_api_alias(), CGEN.module())
...
def conversion_webhooks(git, stage, project, args):
  ...
  webhooks = ConversionWebhooks(args.dryrun, project, args.prev_ver, args.new_ver, CGEN.preferred_api_alias(), CGEN.module())
  webhooks.create()
  CGEN.fix_kubebuilder_import_alias()
  webhooks.hub()
...
def bump_controllers(git, stage, project, args):
  ...
  controllers = Controllers(args.dryrun, project, args.prev_ver, args.new_ver, CGEN.preferred_api_alias(), CGEN.module())
  controllers.run()
...
def bump_apis(git, stage, project, args):
  ...
  controllers = Controllers(args.dryrun, project, args.prev_ver, args.new_ver, CGEN.preferred_api_alias(), CGEN.module(), "api")
...
CGEN = ConversionGen(args.dryrun, project, args.prev_ver, args.new_ver, args.most_recent_spoke)
...
if args.most_recent_spoke:
  if not CGEN.is_spoke(args.most_recent_spoke):
    ...
    sys.exit(1)
  if args.most_recent_spoke == args.prev_ver or args.most_recent_spoke == args.new_ver:
