...
class MvWebhooks:
        ...
        for root, _, f_names in os.walk(f"api/{self._new_ver}", followlinks=False):
            for fname in f_names:
                if "webhook" in fname:
                    ...
                    fu.replace_in_file(
                        f"versions={self._prev_ver},", f"versions={self._new_ver},"
                    )
                    fu.store()
    ...
    def edit_manifests(self):
        ...
        for k in kinds:
            fu.replace_in_file(f"- {self._prev_ver}", f"- {self._new_ver}")
            fu.replace_in_file(
                f"-{self._prev_ver}-{k.lower()}", f"-{self._new_ver}-{k.lower()}"
            )
        fu.store()
    ...
    def mv_project_webhooks(self):
        ...
        if kinds != kinds_new_ver:
            print(
                f"The {self._prev_ver} list of types does not match the {self._new_ver} list"
            )
    ...
    def commit(self, git, stage):
        ...
        msg = (
            f"""Move the existing webhooks from {self._prev_ver} to {self._new_ver}."""
        )
...
class ConversionWebhooks:
    ...
    def _edit_test_comment(self, kind):
        ...
        has_intro = fu.find_in_file(
            f"""Context("When creating {kind} under Conversion Webhook"""
        )
