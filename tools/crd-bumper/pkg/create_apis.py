...
class CreateApis:
    ...
    for k in kinds:
      if k not in kinds_new_ver:
        ...
        if self._dryrun:
          ...
          print(f"Running: {cmd}")
          child = subprocess.Popen(shlex.split(cmd), shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
          res = child.communicate()
          if child.returncode != 0:
            raise Exception(f"Unable to create API for {k}.{self._new_ver}:\n{res[1]}")
  ...
  def copy_content(self, git):
    ...
    skip = ['doc.go', 'groupversion_info.go', 'conversion.go',
            'conversion_test.go', 'zz_generated.deepcopy.go',
    ...
    for root, _, f_names in os.walk(f"api/{self._prev_ver}", followlinks=False):
      for fname in f_names:
        ...
        dst_path = full_path.replace(f"api/{self._prev_ver}/", f"api/{self._new_ver}/")
        if 'webhook' in fname:
  ...
  def set_storage_version(self):
    ...
    for kind in kinds:
      ...
      if os.path.isfile(fname):
        ...
        if line is None:
          raise Exception(f"Unable to place kubebuilder:storageversion in {fname}")
    ...
    for root, _, f_names in os.walk(f"api/{self._new_ver}", followlinks=False):
      for fname in f_names:
        ...
        if os.path.isfile(full_path):
          ...
          fu.replace_in_file(f"package {self._prev_ver}", f"package {self._new_ver}")
          for k in kinds:
