...
"""Mark the specified CRD version as unserved."""
...
PARSER.add_argument(
    ...
    type=str,
    required=True,
    help="Spoke API version to mark as unserved.",
)
PARSER.add_argument(
    "--repo",
    "-r",
...
def unserve_api(args, project, makecmd, git, preferred_api_alias):
    """Mark the specified API version as unserved."""

    unserve = Unserve(args.dryrun, project, args.spoke_ver, preferred_api_alias)

    print(f"Updating files to mark API {args.spoke_ver} as unserved.")

    unserve.set_unserved()
    unserve.modify_conversion_webhook_suite_test()
    ...
    unserve.commit(git, "unserve-api")
