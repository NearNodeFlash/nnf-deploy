...
The entire release process is orchestrated by `release-all.sh` (located in `tools/release-all/`). It handles cloning, branching, PR creation, merging, and tagging for all 10 NNF repos. The helper function `nnf_cmd` wraps `release-all.sh` with the correct flags.
